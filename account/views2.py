...
from account.forms import BookmarksSortingForm, FriendsCommentingForm, \
    FriendsSortingForm, SettingsForm, UploadPhotoForm
from account.models import GameDirection, Game, Direction, Friendship, Profile, \
    Region, LevelUp
...
class FriendsView(View):
    ...
    def get(self, request, page=None, *args, **kwargs):
        ...
        if request.user.is_authenticated:
            ...
            #     else:
            #         status = friendship.status
            #         if status == 0 and friends_sorting == 3:  # на рассмотрении
            #             friendships.append(friendship)
            #         elif status == 1 and friends_sorting == 4:  # взаимные
            #             friendships.append(friendship)
            #         elif status == 3 and friends_sorting == 5:  # полученные заявки
            #             friendships.append(friendship)
            # if friends_sorting == 1:  # по новым
            #     friendships = friendships[::-1]
            # elif friends_sorting == 2:  # по имени - тут лютое количество запросов по полной базе друзей
